...
class MySQLDriver extends DatabaseMethods implements DatabaseInterface
{
    ...
    //User Register
    public function add($pdo, $username, $salary, $city, $password, $email)
    {
        //$result = array();
        //$dsn = "mysql:host=$this->host;dbname=$this->db;charset=utf8mb4";
        $options = [
        ...
        try {
            //$pdo = new PDO($dsn, $this->uid, $this->password, $options);
            $stmt = $pdo->prepare(
                "insert into users(name,salary,city,password,email) values(:username,:salary,:city,:password,:email)"
            );
            $stmt->bindParam(":username", $username);
    ...
    //Pagination
    public function pagination($pdo, $offset, $limit)
    {
        $result = [];
        //$dsn = "mysql:host=$this->host;dbname=$this->db;charset=utf8mb4";
        $options = [
